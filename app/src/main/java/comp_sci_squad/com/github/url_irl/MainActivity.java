...
public class MainActivity extends AppCompatActivity implements
  ...
  /**
   * Extra Name Constants for the ListURLsActivity Intent PICTURE_EXTRA labels a byte array
   ...
   * <p>
   * MyOrientationEventListener extends OrientationEventListener and calls onOrientationChanged() if
   * phone is rotated 55 degrees away from orientation
   */
  ...
  private final View.OnClickListener mOnClickListener = new View.OnClickListener() {
    ...
    public void onClick(View v) {
      ...
      if (v.getId() == R.id.shutter_button) {
        if (mCameraController != null || mEmulatorPreview != null) {
  ...
  /**
   * Calculates the devices current rotation and compares to current orientation and updates UI
   * accordingly.
   *
   * If the difference is greater than 55 degrees it calculates new orientation and and creates an
   * animation for the rotation of the UI elements. It then rotates the UI elements with that
   * animation. Finally, it saves the new orientation and UI element rotation to member variables.
   *
  ...
  /**
   * Create an options menu. It will create - Torchlight button
   *
  ...
  /**
   * Handles menu items being selected. If the item was share_all, start the share chooser with all
   * the urls.
   *
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    if (item.getItemId() == R.id.torch_menu_item) {
      Integer torchState = mCameraController.getTorchState().getValue();
      if (torchState == null) {
  ...
  /**
   * Keeps requesting camera permission til accepted. Once accepted, initialize the Camera View
   *
   * @param requestCode - The request code for permission. Only acts if its
  ...
  /**
   * Compresses a bitmap so its length and width are 1/8 the size Results in a 1/64th size image
   *
  ...
  private byte[] compressBitmap(Bitmap image) {
    ...
    int thumbnailHeight =
        image.getHeight() / getResources().getInteger(R.integer.THUMBNAIL_SHRINK_RATIO);
    int thumbnailWidth =
        image.getWidth() / getResources().getInteger(R.integer.THUMBNAIL_SHRINK_RATIO);
  ...
  /**
   * Calculates the power of 2 the bitmap size must be reduced by to fit within the current JVM
   * Heap. Assumes there will be other allocations and that 2 bitmaps of the size will be
   * allocated.
   *
  ...
  private int getInSampleSize(int width, int height) {
    ...
    long availableMem = rt.maxMemory() - rt.totalMemory() - rt.freeMemory();
  ...
  private void setUpCameraViews() {
    if (!mEmulated) {
      ...
      mCameraController = new LifecycleCameraController(getApplicationContext());
      mCameraController.bindToLifecycle(this);
      mCameraPreview.setController(mCameraController);
    } else {
      ...
      GlideApp.with(this).asBitmap().load(R.raw.tester_pic_remove_on_release)
          .into(new SimpleTarget<Bitmap>() {
            @Override
  ...
  protected class TextRecognitionTask extends AsyncTask<Bitmap, Integer, ArrayList<String>> {
    ...
    /**
     * Enables progress bar on UI thread before task starts. Called automatically by the AsyncTask.
     */
    @Override
    ...
    protected ArrayList<String> doInBackground(Bitmap... params) {
      ...
      ArrayList<String> result = new ArrayList<>(
          ImageToString.getTextFromPage(mContext, params[0]));
    ...
    /**
     * Resets UI elements before starting ListUrlsActivity. Called automatically by the AsyncTask.
     *
    ...
    protected void onPostExecute(ArrayList<String> mParsedText) {
      ...
      Intent intent = ListURLsActivity
          .newIntent(mContext, mParsedText, mThumbnail, mTimeImageTaken);
  ...
  /**
   * Class for GlideApp to Transform bitmaps. The bitmap pool isn't correctly used but most likely
   * isn't needed because of the size of the bitmaps involved.
   */
  private class RotateTransformation extends BitmapTransformation {
    ...
    /**
     * Constructor for the rotation transformation. The class is designed to be reused with multiple
     * transformations.
     *
    ...
    protected Bitmap transform(@NonNull BitmapPool pool, @NonNull Bitmap toTransform, int outWidth,
      ...
      return Bitmap.createBitmap(toTransform, 0, 0, toTransform.getWidth(), toTransform.getHeight(),
          rotationMatrix, true);
    }
  ...
  private class MyOrientationEventListener
    ...
    /**
     * The orientationListener uses Android's Sensor Manager in the background It calls
     * MainActivity's onOrientationChanged if the orientation isn't flat (ORIENTATION_UNKNOWN)
     *
     * @param orientation - the orientation of the device as an angle. Vertical is 0 clockwise to
     * 359
     */
    @Override
    public void onOrientationChanged(int orientation) {
